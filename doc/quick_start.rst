...
To create your package, you need to clone the ``popsregression`` repository:

.. prompt:: bash $

  git clone https://github.com/scikit-learn-contrib/popsregression.git

Before to reinitialize your git repository, you need to make the following
changes. Replace all occurrences of ``popsregression``, ``popsregression``, or
``popsregression`` with the name of you own project. You can find all the
occurrences using the following command:

.. prompt:: bash $

  git grep popsregression
  git grep popsregression
  git grep popsregression
