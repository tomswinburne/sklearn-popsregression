"""
The :mod:`popsregression.utils.discovery` module includes utilities to discover
objects (i.e. estimators, displays, functions) from the `popsregression` package.
"""
...
def all_estimators(type_filter=None):
    """Get a list of all estimators from `popsregression`.
    ...
    Examples
    --------
    >>> from popsregression.utils.discovery import all_estimators
    ...
    all_classes = []
    root = str(Path(__file__).parent.parent)  # popsregression package
    # Ignore deprecation warnings triggered at import time and from walking
    # packages
    with ignore_warnings(category=FutureWarning):
        for _, module_name, _ in pkgutil.walk_packages(
            path=[root], prefix="popsregression."
        ):
...
def all_displays():
    """Get a list of all displays from `popsregression`.
    ...
    Examples
    --------
    >>> from popsregression.utils.discovery import all_displays
...
def _is_checked_function(item):
    ...
    mod = item.__module__
    if not mod.startswith("popsregression.") or mod.endswith("estimator_checks"):
        return False
